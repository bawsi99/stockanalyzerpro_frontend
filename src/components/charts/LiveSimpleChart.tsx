...
import { RefreshCw, Activity, Settings, TrendingUp, TrendingDown } from 'lucide-react';
...
const LiveSimpleChart: React.FC<LiveSimpleChartProps> = ({ 
  ...
  useEffect(() => {
    // Destroy existing chart if symbol changes
    if (chartRef.current) {
      console.log('Destroying existing chart for symbol change');
    ...
    const initializeChart = async () => {
      try {
        ...
        const chart = await initializeChartWithRetry(
          ...
          { width, height, ...createChartTheme(theme === 'dark', { timeframe }) }
        );
    ...
    initializeChart();
  }, [symbol, data, theme, width, height, timeframe, showVolume]); // Add symbol to dependencies

  // Enhanced chart data update with validation
